...
void GraphSalesman::renderGraph(std::string filename, int w, int h,
    ...
    for (auto n = nodesBegin(); n != nodesEnd(); n++) {
        ...
        for (auto m = a.begin(); m != a.end(); m++) {
            ...
            doc << Line(Point(n->first->east, h - n->first->south),
                        Point(x, h - y), Stroke(1, Color::Silver));
        }
    }
    for (auto n = nodesBegin(); n != nodesEnd(); n++) {
        doc << Circle(Point(n->first->east, h - n->first->south), 8,
                      Fill(Color::Black), Stroke(1, Color::Black));
        doc << Text(Point(n->first->east - 10, h - n->first->south + 13),
                    n->first->name, Color::Black, Font(10, "Verdana"));
    }
    if (path) {
        for (int i = 0; i < path->size() - 1; i++) {
            ...
            doc << Line(Point(n->east, h - n->south),
                        Point(m->east, h - m->south), Stroke(3, Color::Green));
            doc << Text(Point(n->east + 10, h - n->south + 0),
                        std::to_string(i), Color::Green, Font(10, "Verdana"));
        }

        float pSize = pathSize(*path);

        doc << Text(Point(10, h - 20), std::to_string(pSize) + " km",
                    Color::Green, Font(10, "Verdana"));
    }
    doc.save();
